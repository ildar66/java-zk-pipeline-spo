...
public class _DBFlexWorkflowHome_Stub extends Stub implements DBFlexWorkflowHome {
    
    private static final String[] _type_ids = {
        "RMI:org.uit.director.db.ejb.DBFlexWorkflowHome:0000000000000000", 
    ...
    public DBFlexWorkflow create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (DBFlexWorkflow) in.read_Object(DBFlexWorkflow.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",org.uit.director.db.ejb.DBFlexWorkflowHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    DBFlexWorkflow result = ((org.uit.director.db.ejb.DBFlexWorkflowHome)so.servant).create();
                    return (DBFlexWorkflow)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
