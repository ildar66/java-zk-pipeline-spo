...
<%@page import="ru.md.persistence.DepartmentHistoryMapper"%>
...
//count
Long totalCount = SBeanLocator.singleton().getDepartmentHistoryMapper().getDepartmentHistoryCount(filter);
 %>
<title>История изменения Инициирующего подразделения в Заявке № <%=number %></title>
...
<div align="left" style="font-size: 22px;"><b>История изменения Инициирующего подразделения в Заявке № <%=number %></b></div>
...
<%} else {
	//list
	//List<ru.md.domain.DepartmentHistory> list = SBeanLocator.singleton().getDepartmentHistoryMapper().getDepartmentHistory(filter);
%>
<div class="paging">
...
<%
	...
	List<ru.md.domain.DepartmentHistory> list = SBeanLocator.singleton().getDepartmentHistoryMapper().getDepartmentHistoryPage(filter, pageSize * (new Integer(pagenum) - 1), pageSize);
%>
...
<%
    Long pageCount = 1 + (totalCount - 1) / pageSize;
        ...
        String link = "departmentChangeReport.jsp?idmdtask=" + (request.getParameter("idmdtask") == null ? "" : request.getParameter("idmdtask"))
...
<table class="regular" border="1">
	<thead>
		<tr>
			<th>Подразделение</th>
			<th>Менеджер</th>
			<th><nobr>Дата и время</nobr></th>
			<th>Версия</th>
		</tr>
	</thead>
	<tbody>
		<%
		    for (ru.md.domain.DepartmentHistory history : list) {
		%>
		<tr <%=(history.getIdmdtask().equals(idTaskMain) ? " style='color:blue' " : "")%> >
			<td>
				<%=history.getNewDepartmentName()%>
			</td>
