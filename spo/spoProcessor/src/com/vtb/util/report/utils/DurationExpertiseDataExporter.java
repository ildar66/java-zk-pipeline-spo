...
/**
 * {@link Генератор} электронной таблицы Сроки проведения экспертиз  
 * {@link StandardPeriod }
 */
public class DurationExpertiseDataExporter extends AExcelDataExporter<StandardPeriod> {
	...
	public DurationExpertiseDataExporter(String mdtaskNumber) {
		super();
	...
	protected Sheet fillCaption(Sheet sheet) throws Exception {
		try {
			...
			sheet.setColumnWidth(columnIndex, 18 * 256);
			cell = row.createCell(columnIndex++);
			fillCell("История событий", style, cell);

			sheet.setColumnWidth(columnIndex, 18 * 550);
			cell = row.createCell(columnIndex++);
			fillCell("Комментарии Экспертного подразделения", style, cell);
	...
	protected Row fillRow(StandardPeriod tuple, Row row) throws Exception {
		try {
			...
			if (tuple == null) throw new NullPointerException("tuple is null");
			...
			// История события
			String events = concatenateString(tuple.getEventsHistory(), NEW_LINE);
			cell = row.createCell(columnIndex++);
			fillCell(events, style, cell);
			
			
			// Комментарии Экспертного подразделения
			String comments = concatenateString(tuple.getComments(), NEW_LINE);
			cell = row.createCell(columnIndex++);
			fillCell(comments, style, cell);
